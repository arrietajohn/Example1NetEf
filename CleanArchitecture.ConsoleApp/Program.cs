using MyProjectDotNet.Data;
using MyProjectDotNet.Domain;
...
async Task QueryMethods()
{
    ...
    // atermativa mas corta
    var firstAsync2 = await streamer.FirstAsync(s => s.Nombre.Contains("a"));
...
async Task QueryFilter()
{
    ...
    /*
    * Alternativa 1
    var streamersResult2 = await (from s in dbContext.Streamers
                                  where s.Nombre.Equals(streamingNombre)
                                  select s).ToListAsync();
    * Alternativa 2
      var streamers2 = from s in dbContext.Streamers
                     where s.Nombre.Equals(streamingNombre)
                     select s;
    var streamersResult = await streamers2.ToListAsync();
    */
    ...
    /*
    Alternativa 1
    var streamerPartialResults2 = await (
                                           from s in dbContext.Streamers
                                           where EF.Functions.Like(s.Nombre, $"%{streamingNombre}")
                                           select s
                                   ).ToListAsync();
    */
